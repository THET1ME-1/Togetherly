...
abstract final class MigrationConfig {
  ...
  // ──────────────────────────────────────────────────────────────
  // ⛔ ОТКАТ 2026-06-22: публичные юзеры — ТОЛЬКО Firebase (как до миграции).
  // Причина: Supabase (supabase.co/AWS) недоступен у части провайдеров РФ →
  // полностью мигрированные пары читали пустоту = «данные пропали» (хотя данные
  // целы в Firebase). Работающих через Supabase мало → отказываемся от него для
  // публики целиком: false → публичные НЕ пишут и НЕ читают Supabase (нет дуал-
  // райта, нет открытой базы через publishable-ключ в APK, нет RLS-шума). Их
  // источник истины — Firebase, как и был. Миграция остаётся живой ТОЛЬКО для 3
  // whitelist-аккаунтов (_phase1Emails) — для будущих тестов. Возврат: true.
  // Компилируется в APK → доходит до юзеров только с новой сборкой.
  ...
  // ──────────────────────────────────────────────────────────────
  // ⛔ ОТКАТ 2026-06-22: ВСЕ читают из Firebase. Причина — Supabase
  // (supabase.co, AWS) недоступен у части провайдеров РФ (блокировка): полностью
  // мигрированные пары читали ПУСТОЙ Supabase = «данные пропали», хотя данные
  // целы в Firebase (дуал-райт держался, т.к. живая проба _sbReadVerified не
  // подтверждалась → _writeFb не срезал Firebase-запись). false здесь —
  // мгновенный откат ВСЕХ чтений на Firebase + авто-возврат Firebase-записи
  // (_writeFb завязан на _readSb). Дуал-райт в Supabase теперь только у 3
  // whitelist-аккаунтов (enabledForEveryone=false), публичные — чистый Firebase.
  // ВАЖНО: флаг компилируется в APK → доходит до юзеров ТОЛЬКО с новой сборкой,
  // не серверным переключением.
  static const bool stage3ReadFromSupabase = false;
