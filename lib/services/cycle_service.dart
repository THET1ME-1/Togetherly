...
class CycleService extends ChangeNotifier {
  ...
  ///
  /// Условие одно — Togetherly+, потому что это платная часть. Пол не
  /// спрашиваем с 08.09.2026: раньше карточку видел только женский профиль, и
  /// человек с пустым или другим полом не находил цикл вовсе, а объяснения
  /// ниоткуда не получал — при входе через Google пол легко остаётся не
  /// выбранным. Кому отметки не нужны, тот их не заводит; кому нужны — ведёт,
  /// каким бы ни был пол в профиле.
  ///
  /// Исключение — платформа, где Togetherly+ не продаётся: там некупившему
  /// раздела нет вовсе, вести его некуда. У купившего он на месте: флаг живёт
  /// на аккаунте и переезжает вместе с ним.
  static bool availableFor(UserData? user) => PlusService.instance.visible;
