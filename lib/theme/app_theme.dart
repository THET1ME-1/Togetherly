...
class AppTheme {
  ...
  /// Цвет ЗАЛИВКИ акцентного/активного элемента, ПОВЕРХ которого лежит текст
  /// или иконка: FAB, круглые кнопки, заполненные лепестки таймера, ячейка
  /// «сегодня», активный пункт навигации.
  ///
  /// Заливка — это всегда сам цвет темы. Раньше светлый акцент в тёмной теме
  /// подменялся графитом: сработало на семи палитрах из 25 (Мятная, Лимонная,
  /// Бирюзовая, Медовая, Персиковая, Лавандовая, Песочная), и купленная за
  /// монеты «Мятная» в тёмном режиме становилась серой. Подмена осталась от
  /// времён, когда текст поверх заливки был жёстко белым; теперь его считает
  /// [AppThemes.onColor] по настоящему контрасту WCAG, и на светлой мяте он
  /// сам берётся тёмным. Пятна «белым по белому» больше не бывает.
  Color get fillColor => accentFill ?? primary;

  /// Трек лепесткового круга — цвет темы, притемнённый.
  ///
  /// Раньше он был бледной подложкой (яркость 0,53–0,70), и цифры на нём
  /// приходилось писать тёмными: белым выходило 1,4–1,8, то есть никак. Круг
  /// занимает половину главного экрана, и половина его подписей шла вразрез с
  /// правилом «на цветном пишем белым».
  ///
  /// Тридцать восемь процентов чёрного — не подбор на глаз: столько нужно,
  /// чтобы белый уверенно читался у самой светлой палитры (мятная), и при этом
  /// заполненный сектор остаётся заметно светлее трека у всех остальных.
  ///
  /// В тёмных темах трек не трогаем: там он и так спокойная поверхность, а
  /// заполненный сектор читается цветом.
  ///
  /// Отдельное поле, а не переделка `timerDialBackground`: тем же цветом
  /// красится пилюля «сегодня» в календаре настроений, и её притемнять нельзя —
  /// число дня на ней написано акцентом.
  Color get dialTrack => brightness == Brightness.dark
      ? timerDialBackground
      : Color.alphaBlend(const Color(0x61000000), fillColor);
}
...
abstract final class AppThemes {
  ...
  // Кремово-нейтральный текст, чёткие рёбра: свечений в приложении больше нет.
  ...
  // рёбра, никаких свечений. Полный набор тёмных токенов — виджеты
  ...
  /// (`readable_text.dart`).
  /// В светлых темах поверх цветного пишем БЕЛЫМ.
  ///
  /// Решение заказчика от 11 августа 2026: чёрные подписи на персиковой,
  /// закатной и розовой заливке выглядели дёшево. По контрасту белый там
  /// слабее (2,4–3,2 против 5,8–7,4 у чёрного), и автоподбор выбирал чёрный —
  /// поэтому правило теперь задано прямо, а не считается.
  ///
  /// Чёрный остаётся на совсем светлых заливках: светлота 0,41 и выше — это
  /// тон 70 по HCT (мятная 72, медовая 71), там белым не разобрать даже
  /// вывеску. Считаем яркостью, чтобы не тащить сюда цветовую библиотеку.
  ///
  /// Тёмные темы работают как раньше, по контрасту: их заливки — пастель тона
  /// 80, белый по ним даёт 1,7 и пропадает начисто.
  static Color onColor(Color background, {Brightness? mode}) {
    if (mode == Brightness.light && background.computeLuminance() < 0.407) {
