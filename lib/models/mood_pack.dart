...
///
/// [buysInStore] — можно ли купить его прямо здесь, через биллинг магазина.
/// На iPhone это стало возможно 19.08.2026: товар `mood_pack.moti` заведён в
/// App Store Connect. Пока товара не было, платный набор там прятали целиком —
/// вести на внешнюю оплату запрещает 3.1.1, а витрина с ценой без покупки
/// только дразнит.
///
/// Уже открытый набор виден на iPhone в любом случае: купить его могли и на
/// Android, и на сайте, а покупка общая на пару. Отбирать оплаченное из-за
/// платформы нельзя.
bool moodPackVisible({
  required bool isIOS,
  required bool isMoney,
  required bool isOpen,
  bool buysInStore = false,
}) =>
    !(isIOS && isMoney && !isOpen && !buysInStore);
