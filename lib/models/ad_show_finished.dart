...
/// То же для показа внутри окна приложения, где ухода в фон не было.
///
/// Замер 26.08.2026 по шестидесяти последним потерянным наградам: `away` не
/// приходит вовсе (реклама Яндекса рисуется без смены жизненного цикла), а
/// медиана времени на экране — 120 секунд. Прежняя граница в семьдесят секунд
/// (предохранитель ожидания) отвергала ровно тех, кто досмотрел ролик:
/// 15 601 событие «монет нет» и ни одного случая «закрыл сам».
///
/// Экономику держит не эта граница, а серверный предел начислений в сутки.
const Duration kAdOnScreenSane = Duration(minutes: 5);
...
bool adRewardDeserved({
  ...
  // времени. Верхняя граница здесь СВОЯ и щедрая: предохранитель ожидания
  // (70 с) отсекал большинство досмотревших — у них медиана 120 секунд.
  if (away == Duration.zero) {
    return onScreen >= kAdWatchedEnough && onScreen <= kAdOnScreenSane;
  }
  return false;
}
