...
(async () => {
  ...
  // Порядок перевёрнут 21.08.2026. Раньше первым шёл прямой порт 8443 — он
  // обходил перегруженный Caddy. Точка входа с тех пор переехала на отдельный
  // сервер, а нестандартный порт у части операторов МОЛЧА проглатывается:
  // соединение висит до TCP-таймаута, close не приходит, и перебор внутри
  // centrifuge-js не трогается с места. Человек при этом сидит в комнате и не
  // может ничего. Первым идёт путь через 443 — тот же, по которому пришла
  // сама страница, значит он у него точно проходит.
  check('первый — тот же порт, что у страницы', !/:8443\//.test(endpoints[0]),
      endpoints[0] || '');
  check('прямой порт остался запасным',
      endpoints.slice(1).some((e) => /:8443\//.test(e)),
